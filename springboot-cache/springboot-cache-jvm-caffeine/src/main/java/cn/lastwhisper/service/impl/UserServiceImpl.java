...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
     ...
     *  2、默认缓存配置 org.springframework.boot.autoconfigure.cache.SimpleCacheConfiguration
     *  3、SimpleCacheConfiguration 为容器中注册 CacheManager（默认 ConcurrentMapCacheManager）
     *  4、通过 cacheNames 与 Cache（默认 ConcurrentMapCache）关联
     */
     ...
     *      ConcurrentMapCacheManager.getCache
     ...
     *    查看ConcurrentMapCache的lookup和put方法
    ...
    @Caching(
            cacheable = {
                    @Cacheable(/*cacheNames = "user",*/key = "#user.email") //用 email 做缓存 k
            },
            put = {
                    @CachePut(/*cacheNames = "user",*/key = "#result.userId"), //用 userId 做缓存 k
                    @CachePut(/*cacheNames = "user"",*/ key = "#result.phone") //用 phone 做缓存 k
            }
    )
    @Override
